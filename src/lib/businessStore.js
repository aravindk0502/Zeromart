...
export const toMarketplaceItem = (product, account) => ({
  ...
  sellerId: account?.profileId || account?.userId || account?.id || product.businessId || '',
  ownerMobile: account?.mobile || product.businessId || account?.id || '',
  sellerKarma: account?.karma || 0,
  sellerAvatar: account?.profileImage || account?.avatarUrl || '',
  sellerProfileImage: account?.profileImage || account?.avatarUrl || '',
  avatarUrl: account?.profileImage || account?.avatarUrl || '',
  ...
  sellerProfile: {
    id: String(account?.profileId || account?.userId || account?.id || '').trim(),
    name: account?.businessName || product.storeName || 'Business Store',
    initials: String((account?.businessName || product.storeName || 'BS').split(' ').map((part) => part[0] || '').join('').slice(0, 2) || 'BS').toUpperCase(),
    avatarUrl: account?.profileImage || account?.avatarUrl || '',
    logoUrl: account?.profileImage || account?.avatarUrl || '',
    accountType: 'business',
    area: account?.locationData?.area || account?.locationData?.locality || '',
    city: account?.locationData?.city || '',
    karma: Number(account?.karma ?? 0) || 0,
    activeListings: 0,
